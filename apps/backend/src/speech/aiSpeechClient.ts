...
export const aiSpeechClient = {
  async transcribe(audioBase64: string, targetLanguage: string = "english"): Promise<string> {
    const res = await fetch(`${AI_SERVICE_URL}/v1/speech/transcribe`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ audioBase64, targetLanguage }),
    });
  ...
  async synthesize(
    ...
    const res = await fetch(`${AI_SERVICE_URL}/v1/speech/synthesize`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text, voice, targetLanguage }),
    });
    if (!res.ok) throw new Error(`AI service returned ${res.status}`);
